...
highlights_menu_status_message.className = "highlights_area_item";
...
settings_menu.appendChild(save_settings_button);
save_settings_button.textContent = "SAVE SETTINGS";

function update_main_menu() {
	if(current_processing_state === 1){
		highlights_menu_status_message.textContent = "Gathering Chat Messages";
	} else if(current_processing_state === 2) {
		highlights_menu_status_message.textContent = "Analyzing Chat Messages";
	} else {
...
function get_next_continuation(continuation_id, message_array, iteration_count) {
	...
	.then(
		(data) => {
			...
			if(data)
				get_next_continuation(chat_info.continuations[0].liveChatReplayContinuationData.continuation, message_array, iteration_count + 1);
		}
	);
}
...
const recommended_section = document.getElementById("related");
recommended_section.parentNode.insertBefore(highlights_area, recommended_section);
